  ...
  Future<void> subscribeToRoom(
    ...
    try {
      ...
      _gameChannel!.onPresenceSync((payload) {
        ...
        for (final presenceGroup in presences) {
          for (final presence in presenceGroup.presences) {
            final presenceMap = presence.payload;

            // Try to find the Game ID in order of reliability
            final gamePlayerId = presenceMap['id'] as String? ??
                presenceMap['player_id'] as String? ??
                presenceGroup.key; // Fallback to session key
